...
const defaultProvider = 'http://127.0.0.1:8732',
  ...
  crypto = {
        ...
        case 'edsk':
          if (sk.length == 98) {
            ...
            const privateKey = new Buffer(
              library.sodium.crypto_sign_SECRETKEYBYTES
            )
            const publicKey = new Buffer(
              library.sodium.crypto_sign_PUBLICKEYBYTES
            )
    ...
    generateKeysFromSeedMulti: function (m, p, n) {
      n /= 256 ^ 2
      const s = library.bip39
        .mnemonicToSeedSync(
          m,
          library.pbkdf2
            .pbkdf2Sync(p, n.toString(36).slice(2), 0, 32, 'sha512')
            .toString()
        )
        .slice(0, 32)
    ...
    sign: function (bytes, sk, wm) {
      ...
      library.sodium.crypto_sign_detached(
        sig,
        out,
...
;(node = {
  ...
  query: function (e, o, t) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      http.onload = function () {
        if (http.status === 200) {
          if (node.debugMode) console.log(http.status, e, o, http.responseText)
  ...
  (rpc = {
    ...
    getPlexBalance: function (mp1, block = 'head') {
      return node
        .query(`/chains/main/blocks/${block}/context/contracts/${mp1}/balance`)
        .then(function (r) {
          return r
        })
    },
    getMineBalance: function (mp1, block = 'head') {
      return node
        .query(
          `/chains/main/blocks/${block}/context/contracts/${mp1}/mine_balance`
        )
        .then(function (r) {
          return r
        })
    ...
    getConstants: function (block = 'head') {
      return node
        .query(`/chains/main/blocks/${block}/context/constants`)
        .then(function (c) {
          if (c) return c
          return false
        })
    ...
    getDelegate: function (mp1, block = 'head') {
      return node
        .query(`/chains/main/blocks/${block}/context/contracts/${mp1}/delegate`)
        .then(function (r) {
          if (r) return r
          return false
        })
    ...
    getDelegateInfo: function (mp1, block = 'head') {
      return node
        .query(`/chains/main/blocks/${block}/context/delegates/${mp1}`)
        .then(function (r) {
          if (r) return r
          return false
        })
    ...
    getDelegatedAddresses: function (mp1, block = 'head') {
      return node
        .query(
          `/chains/main/blocks/${block}/context/delegates/${mp1}/delegated_contracts`
        )
        .then(function (r) {
          if (r) return r
          return false
    ...
    getOwnStakingMineBalance: function (mp1, block = 'head') {
      return node
        .query(`/chains/main/blocks/${block}/context/delegates/${mp1}/balance`)
        .then(function (r) {
          if (r) return r
          return false
        })
    ...
    getDelegatedBalance: function (mp1, block = 'head') {
      return node
        .query(
          `/chains/main/blocks/${block}/context/delegates/${mp1}/delegated_balance`
        )
        .then(function (r) {
          if (r) return r
          return false
    ...
    getStakingMineBalance: function (mp1, block = 'head') {
      return node
        .query(
          `/chains/main/blocks/${block}/context/delegates/${mp1}/staking_balance`
        )
        .then(function (staking_balance) {
    ...
    getAllActiveDelegates: function (block = 'head') {
      return node
        .query(`/chains/main/blocks/${block}/context/delegates?active=true`)
    ...
    getBlockBakingRights: function (block = 'head', maxPriority = 999) {
      return node
        .query(
          `/chains/main/blocks/head/helpers/baking_rights?level=${block}&max_priority=${maxPriority}`
        )
        .then(function (info) {
          return info
        })
    ...
    getBlockEndorsingRights: function (block = 'head') {
      return node
        .query(
          `/chains/main/blocks/head/helpers/endorsing_rights?level=${block}`
        )
        .then(function (info) {
          return info
        })
    ...
    getCurrentCycle: function (block = 'head') {
      return node
        .query(`/chains/main/blocks/${block}/metadata`)
        .then(function (info) {
    ...
    getLevelsInCurrentCycle: function (block = 'head') {
      return node
        .query(`/chains/main/blocks/${block}/helpers/levels_in_current_cycle`)
        .then(function (info) {
          return info
        })
        .catch(function () {
    ...
    sendOperation: function (from, operation, keys) {
      ...
      for (let i = 0; i < ops.length; i++) {
        if (
          ...
          promises.push(
            node.query(
              '/chains/main/blocks/head/context/contracts/' + from + '/counter'
            )
          )
          promises.push(
            node.query(
    ...
    getPricePlexForOneMine: async (total_stake = 0, block = 'head') => {
      ...
      if (!constants) throw new Error('error load constants')
    ...
    originate: function (
      ...
      var _code = utility.ml2mic(code),
        ...
        operation = {
          ...
          delegate:
            typeof delegate != 'undefined' && delegate ? delegate : keys.pkh,
          script: script,
        }
      return rpc.sendOperation(from, operation, keys)
    },
    ...
    hashData(data, type) {
      ...
      return node.query(
        '/chains/main/blocks/head/helpers/scripts/hash_data',
        check
      )
    },
  ...
  (contract = {
    ...
    send: function (contract, from, keys, amount, parameter, fee) {
      return mpapi.rpc.sendOperation(
        from,
        {
          kind: 'transaction',
          fee: fee.toString(),
